...
export class ObjectPool<T> {
  ...
  recycle(object: T): void {
    ...
    // shrink the pool of available objects if it is larger than 20% of the total size
    if (this.availableObjects.length > Math.round(this.size * 0.2)) {
      this.free(this.availableObjects.length - Math.round(this.size * 0.2))
    }
  }
}

...
export const prepareQuery = (
  ...
  if (
    queryBuilderConditions.some((condition) => condition.components.length <= 0)
  ) {
  ...
  const combinedAllCondition: QueryCondition<any> = {
    type: 'all',
    components: [],
  };
  const combinedNotCondition: QueryCondition<any> = {
    type: 'not',
    components: [],
  };

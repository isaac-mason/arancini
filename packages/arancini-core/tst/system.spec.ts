...
describe('System', () => {
  ...
  test('systems can attach other systems', () => {
    class TestSystemWithAttachedSystem extends System {
      attachedSystem = this.attach(SystemOne)
    }
    ...
    expect(system.attachedSystem).toBeTruthy()
    expect(system.attachedSystem).toBeInstanceOf(SystemOne)

    world.unregisterSystem(SystemOne)

    expect(system.attachedSystem).toBe(undefined)
  })
})

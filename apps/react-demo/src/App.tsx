...
class LinearVelocitySystem extends System {
  linvel = this.query([LinearVelocity, ThreeComponent]);

  onUpdate(delta: number): void {
    for (const entity of this.linvel) {
      const { object } = entity.get(ThreeComponent);
      const { linvel } = entity.get(LinearVelocity);
...
world.registerComponent(LinearVelocity);
  ...
  return (
    <>
      <ECS.Entity>
        ...
        <ECS.Component type={LinearVelocity} args={[0.25, 0.5, 1]} />
      </ECS.Entity>
